...
const FilterationModal: React.FC<FilterationModalProps> = ({ handleClose }) => {
  const { setFilteration: setFilters, filteration: filters } =
  ...
  return (
    <div className="bg-gray-200 p-5 shadow-lg rounded-lg w-[300px]">
      <div className="">
          ...
          {filtersData.map((filter) => (
            <div key={filter.value} className="flex flex-col mt-5">
              ...
              <Select
                ...
                selectedKeys={[
                  filteration.find((f) => f.attribute === filter.value)
                    ?.value ||
        ...
        <button
          className="bg-[#0056B3] text-white px-5 py-2 rounded-lg mt-5"
